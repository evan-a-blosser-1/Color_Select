...
selected_colors = []  # List to store selected colors

# Define themes for each mode
themes = {
    "Space Mode": {
        "bg_color": "#333333",
        "font_color": "cyan",
...
def pick_color():
    ...
    if color_code:
        # Update the label with the selected color code
...
def start_color_picker():
    # Create a top-level window for the floating cursor
    picker_window = tk.Toplevel(root)
    picker_window.attributes('-fullscreen', True)
    picker_window.attributes('-alpha', 0.3)  # Make the window semi-transparent
    ...
    picker_window.bind("<Button-1>", on_click)  # Left-click to select color
...
def show_about():
    ...
    if os.path.exists(about_file_path):
        ...
        about_text.config(state="disabled")  # Make the text read-only
...
def show_license():
    ...
    if os.path.exists(license_file_path):
        ...
        license_text.config(state="disabled")  # Make the text read-only
...
# Create a button to start the floating cursor color picker
...
# Create labels to display the selected colors in a 5x2 grid
color_boxes = []
for row in range(5):
    for col in range(2):
        label = tk.Label(root, text="", bg="white", width=10, height=2, font=("Arial", 12))
        canvas.create_window(100 + col * 150, 250 + row * 50, window=label)  # Adjusted coordinates for 2x5 grid
...
# Add theme options to the "Themes" submenu
for mode in modes:
    themes_menu.add_command(label=mode, command=lambda m=mode: switch_theme(m))

# Add "Save Colors" option to the "Settings" menu
file_menu.add_command(label="Save Colors", command=save_colors)

# Add "Load Colorblind Palette" option to the "Settings" menu
...
# Add "MIT License" option to the "Help" menu
